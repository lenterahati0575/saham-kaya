    ...
    if not gj.is_configured():
        ...
        with sub5:
            ...
            if trades_edit.empty:
                ...
                st.markdown("**Data Exit** (kosongkan Harga Exit kalau posisi ini seharusnya OPEN lagi)")
                ec9, ec10 = st.columns(2)
                e_tgl_exit = ec9.text_input("Tanggal Exit (YYYY-MM-DD, kosongkan kalau OPEN)",
                                             value=str(row_edit["Tanggal Exit"] or ""), key="e_tgl_exit")
                e_exit_price = ec10.number_input("Harga Exit (Rp, 0 = OPEN)", min_value=0.0,
