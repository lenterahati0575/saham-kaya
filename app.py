...
from screener import (DEFAULT_PARAMS, load_ticker_universe, fetch_price_history, build_screener_table,
                      build_trade_candidates, classify_daytrading_tipe, fetch_ihsg_history, market_regime, donchian_levels)
...
def _check_auth() -> bool:
    ...
    if not app_password:
        st.warning(
            ...
            "Secrets. Isi `APP_PASSWORD = \"password-rahasia-anda\"` "
            "di Secrets untuk mengunci dashboard ini."
        )
        return True
...
with t_real:
    ...
    if not gj.is_configured():
        ...
        with sub1:
            ...
            for _, r10 in cands_swing_rj.iterrows():
                top10_gabung.append({"label": f"🌊 {r10['Saham']} (Swing) - Entry {r10['Entry']:,.0f} · "
                                               f"SL {r10['Stop Loss']:,.0f} · TP {r10['Target']:,.0f}",
                                      "saham": r10["Saham"], "entry": r10["Entry"],
                                      "sl": r10["Stop Loss"], "target": r10["Target"]})

            def _isi_dari_top10():
                pilihan = st.session_state.get("pilih_top10_rj")
                if pilihan and pilihan != "-- ketik manual --":
                    match10 = next((x for x in top10_gabung if x["label"] == pilihan), None)
                    if match10:
                        st.session_state["saham_rj"] = match10["saham"]
                        st.session_state["entry_rj"] = float(match10["entry"])
                        st.session_state["sl_rj"] = float(match10["sl"])
                        st.session_state["target_rj"] = float(match10["target"])

            if top10_gabung:
                st.selectbox(
                    "🎯 Pilih cepat dari Top 10 Day/Swing (opsional - isi otomatis di bawah, tetap bisa diedit)",
                    options=["-- ketik manual --"] + [x["label"] for x in top10_gabung],
                    key="pilih_top10_rj", on_change=_isi_dari_top10,
                )
            else:
